...
type CustomerInfo struct {
	ExtCorpModel
	ExtCustomerID string                        `gorm:"type:varchar(64);uniqueIndex:idx_ext_staff_id_ext_customer_id;comment:微信客户ID" json:"ext_customer_id"`
	ExtStaffID    string                        `gorm:"type:varchar(64);uniqueIndex:idx_ext_staff_id_ext_customer_id;comment:微信员工ID" json:"ext_staff_id"`
	Age           int                           `gorm:"type:smallint;comment:年龄" json:"age"`
	Description   string                        `gorm:"type:text;comment:描述" json:"description"`
	Email         string                        `gorm:"type:varchar(64);comment:邮箱" json:"email"`
	PhoneNumber   string                        `gorm:"type:varchar(64);comment:电话" json:"phone_number"`
	...
	RemarkField   constants.CustomerRemarkField `gorm:"type:jsonb;comment:自定义字段的值"  json:"remark_values"`
	Timestamp
}
...
func (ci CustomerInfo) Upsert(info CustomerInfo) error {
	return DB.Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"age", "description", "email", "phone_number", "qq", "address", "birthday", "weibo", "remark_field", "updated_at"}),
	}).Create(&info).Error
}
